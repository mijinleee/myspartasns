from django.shortcuts import render, redirect  # render는 html 파일을 화면에 띄어주는 역할을 함
    ...
    if request.method == "GET":
        ...
        if password != password2:
            ...
            if username == "" or password == "":
                ...
                return render(
                    request, "user/signup.html", {"error": "사용자 이름과 패스워드는 필수 값 입니다"}
                )

            exist_user = get_user_model().objects.filter(username=username)
            if exist_user:
                return render(
                    request, "user/signup.html", {"error": "사용자가 존재합니다."}
...
def sign_in_view(request):
    if request.method == "POST":
        ...
        if me is not None:  # 저장된 사용자의 패스워드와 입력받은 패스워드 비교
            ...
            return render(
                request, "user/signin.html", {"error": "유저이름 혹은 패스워드를 확인 해 주세요"}
            )  # 로그인 실패
...
@login_required
def logout(request):
    auth.logout(request)  # 인증되어 있는 정보 없애기
    return redirect("/")
